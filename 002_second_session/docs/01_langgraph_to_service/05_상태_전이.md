...
- 라우터/전이 지점: `src/secondsession/core/chat/graphs/chat_graph.py`
- 요약 전이 결정: `src/secondsession/core/chat/nodes/decide_summary_node.py`
- 폴백 연동: `src/secondsession/core/chat/nodes/fallback_node.py`

---

## 15. 실습 최소 순서(권장)

1) 전이표를 먼저 작성한 뒤 `route_by_safeguard`와 `decide_summary_node`에 반영한다.  
2) **조건 기반 전이**와 **에러 코드 전이**를 분리해 정책 객체로 정리한다.  
3) 전이 결정 시 `route`, `error_code`, `user_message`를 상태에 저장한다.  
4) 전이 로그(메타데이터 이벤트) 발행 규칙을 스트리밍과 연결한다.
