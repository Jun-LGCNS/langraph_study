...
- 병렬 그래프 스켈레톤: `src/secondsession/core/chat/graphs/parallel_chat_graph.py`
- 상태 스키마(병렬 결과 보관): `src/secondsession/core/chat/state/chat_state.py`

---

## 16. 실습 최소 순서(권장)

1) `parallel_chat_graph`에 **팬아웃 → 팬인** 구조를 먼저 만든다.  
2) 배리어/쿼럼 정책을 **함수 또는 정책 객체**로 분리한다.  
3) 부분 실패가 있어도 합류 가능한 기준을 명시한다.  
4) 병렬 호출 비용 상한을 팀 정책으로 문서화한다.
