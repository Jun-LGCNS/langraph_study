...
- 검증 실패/에러 코드: `src/secondsession/core/chat/const/error_code.py`
- 답변 생성 노드(검증/에러 설정): `src/secondsession/core/chat/nodes/answer_node.py`
- 폴백 노드(메시지 결정): `src/secondsession/core/chat/nodes/fallback_node.py`
- 라우팅 분기(에러 코드 반영): `src/secondsession/core/chat/graphs/chat_graph.py`
- 스트리밍 이벤트 스키마: `src/secondsession/api/chat/model/chat_stream_event.py`

---

## 9. 실습 최소 순서(권장)

1) `answer_node`에서 스키마 검증 실패를 **ErrorCode.VALIDATION**으로 변환한다.  
2) `fallback_node`가 에러 코드별 메시지를 반환하도록 구현한다.  
3) 그래프 라우팅에서 에러 코드 존재 시 폴백으로 전환하는 정책을 추가한다.  
4) 스트리밍 단계에서 `error` 이벤트를 **token보다 먼저** 보내는 규칙을 정한다.
