...
- 병렬 그래프 구현 대상: `src/secondsession/core/chat/graphs/parallel_chat_graph.py`
- 병합 결과 저장 상태: `src/secondsession/core/chat/state/chat_state.py`
- 에러 코드/폴백 메시지: `src/secondsession/core/chat/const/error_code.py`

---

## 14. 실습 최소 순서(권장)

1) 팬아웃 노드에서 **후보 2개 이상**을 생성한다.  
2) 팬인 노드에서 **배리어/쿼럼 정책**으로 합류 조건을 검사한다.  
3) 부분 실패 시 **error_code**를 상태에 기록한다.  
4) 합류 결과를 `last_assistant_message`에 저장한다.
