...
아래 파일을 읽어보며 문서 개념을 실제 코드에 연결하세요.

- 폴백 에러 코드: `src/secondsession/core/chat/const/error_code.py`
- 안전 라벨(Enum): `src/secondsession/core/chat/const/safeguard_label.py`
- 폴백 노드: `src/secondsession/core/chat/nodes/fallback_node.py`
- 라우팅 분기(안전 라벨 기준): `src/secondsession/core/chat/graphs/chat_graph.py`
- 상태 스키마/에러 코드 보관: `src/secondsession/core/chat/state/chat_state.py`

---

## 10. 실습 최소 순서(권장)

1) `ErrorCode`와 `SafeguardLabel`의 역할을 읽고, 폴백 메시지 정책을 정리한다.  
2) `fallback_node`에 **에러 코드 → 사용자 메시지** 매핑 로직을 작성한다.  
3) `route_by_safeguard`에서 라벨 기반 분기와 에러 코드 기록을 연결한다.  
4) 폴백 발생 시 스트리밍으로 `error` 이벤트를 보내는 흐름을 설계한다.
