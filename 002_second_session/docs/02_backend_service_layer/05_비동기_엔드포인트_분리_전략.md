...
- 요청/스트리밍/상태/취소 라우터: `src/secondsession/api/chat/router/chat_router.py`
- 서비스 레이어: `src/secondsession/api/chat/service/chat_service.py`
- 상태 응답 모델: `src/secondsession/api/chat/model/chat_job_status_response.py`
- 취소 응답 모델: `src/secondsession/api/chat/model/chat_job_cancel_response.py`

---

## 11. 실습 최소 순서(권장)

1) `POST /chat/jobs`에서 **job_id/trace_id/thread_id**를 반환한다.  
2) `GET /chat/stream/{job_id}`는 SSE로만 응답하도록 고정한다.  
3) `GET /chat/status/{job_id}`는 마지막 `seq`를 반환하도록 설계한다.  
4) `POST /chat/cancel/{job_id}`는 워커가 확인할 수 있는 취소 플래그를 기록한다.
