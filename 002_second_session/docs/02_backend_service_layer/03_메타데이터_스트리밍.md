...
- 워커 이벤트 생산: `src/secondsession/core/worker/chat_worker.py`
...
1) `ChatStreamMetadata`의 `event` 값을 Enum으로 고정한다.  
2) 노드 시작/종료 시점에 메타데이터 이벤트를 생산한다.  
3) 폴백 진입 시 `error_code`와 `safeguard_label`을 포함한다.  
4) `job_queued/job_start/job_end` 이벤트를 워커 흐름에 추가한다.
