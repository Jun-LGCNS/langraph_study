...
- 이벤트 타입 Enum: `src/secondsession/api/chat/const/stream_event_type.py`
- 이벤트 스키마: `src/secondsession/api/chat/model/chat_stream_event.py`
- 메타데이터 스키마: `src/secondsession/api/chat/model/chat_stream_metadata.py`
- 스트리밍 소비자(서비스): `src/secondsession/api/chat/service/chat_service.py`

---

## 12. 실습 최소 순서(권장)

1) `ChatStreamEvent`에 맞춰 **필수 필드(type/trace_id/seq)**를 강제한다.  
2) SSE 직렬화 함수(`to_sse_line`)를 서비스 레이어에 추가한다.  
3) 워커에서 토큰 이벤트마다 `seq`를 증가시키고 `done`을 마지막에 추가한다.  
4) 노드 이름은 이벤트에서 추출 규칙을 정해 일관되게 넣는다.
