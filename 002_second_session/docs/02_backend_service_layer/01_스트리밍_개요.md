...
- 라우터(요청/스트림/상태/취소): `src/secondsession/api/chat/router/chat_router.py`
- 서비스 레이어: `src/secondsession/api/chat/service/chat_service.py`
- 워커 실행: `src/secondsession/core/worker/chat_worker.py`
- 작업/이벤트 큐: `src/secondsession/core/common/queue/chat_job_queue.py`, `src/secondsession/core/common/queue/chat_stream_event_queue.py`
- 스트리밍 스키마: `src/secondsession/api/chat/model/chat_stream_event.py`

---

## 15. 실습 최소 순서(권장)

1) `ChatService.create_job`에서 **job_id/trace_id 생성 → 큐 적재**를 구현한다.  
2) `ChatWorker`가 큐에서 작업을 꺼내 그래프를 실행하도록 연결한다.  
3) 워커가 `token/metadata/error/done` 이벤트를 큐에 적재한다.  
4) `stream_events`에서 큐를 소비해 SSE로 반환한다.
