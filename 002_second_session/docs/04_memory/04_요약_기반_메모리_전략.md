...
- 요약 결정 노드: `src/secondsession/core/chat/nodes/decide_summary_node.py`
- 요약 노드: `src/secondsession/core/chat/nodes/summary_node.py`
- 요약 프롬프트: `src/secondsession/core/chat/prompts/summary_prompt.py`
...
1) `decide_summary_node`에 턴 수 기반 정책을 구현한다.
2) `summary_node`에서 요약 프롬프트를 조합한다.
3) 요약 결과를 summary에 저장한다.
4) 요약 품질 체크리스트를 운영 로그로 남긴다.
