...
- 요청 스키마: `src/secondsession/api/chat/model/chat_job_request.py`
- history DTO: `src/secondsession/core/chat/const/chat_history_item.py`
- 상태 스키마: `src/secondsession/core/chat/state/chat_state.py`
- 히스토리 누적: `src/secondsession/core/chat/nodes/append_history_node.py`

---

## 10. 실습 최소 순서(권장)

1) `ChatJobRequest`의 `query`를 내부 메시지로 정규화한다.
2) `history` 입력도 동일한 포맷으로 변환한다.
3) Tool 메시지 구조를 정의하고 `metadata`에 분리한다.
4) 정규화된 메시지를 `ChatHistoryItem`으로 검증한다.
