...
- Redis 큐: `src/secondsession/core/common/queue/chat_stream_event_queue.py`
- Redis 캐시 예시: `docs/02_backend_service_layer/04_Redis_캐시_rpush_lpop.md`
- 체크포인터 Redis 구현: `src/secondsession/core/common/checkpointer/redis_checkpointer.py`

---

## 10. 실습 최소 순서(권장)

1) session_id/thread_id 기반 키를 정의한다.
2) Redis 리스트에 history를 rpush로 저장한다.
3) lrange로 최근 N개를 조회하는 유틸을 만든다.
4) TTL/아카이빙 정책을 문서화한다.
