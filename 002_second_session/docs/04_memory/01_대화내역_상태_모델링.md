...
- 대화 항목 DTO: `src/secondsession/core/chat/const/chat_history_item.py`
- 히스토리 누적 노드: `src/secondsession/core/chat/nodes/append_history_node.py`
...
1) `ChatState`에 history/turn_count 누적 리듀서를 연결한다.
2) `ChatHistoryItem` 스키마에 role 제한 규칙을 추가한다.
3) `append_history_node`에서 신규 항목만 반환하도록 구현한다.
4) history 누적 후 `turn_count`가 정확히 증가하는지 확인한다.
