...
- 체크포인터: `src/secondsession/core/common/checkpointer/inmemory_checkpointer.py`
- Redis 체크포인터: `src/secondsession/core/common/checkpointer/redis_checkpointer.py`

---

## 10. 실습 최소 순서(권장)

1) `thread_id`를 요청/응답에 포함시킨다.
2) 동일 thread_id로 재요청해 복구가 되는지 확인한다.
3) 동시 요청 시 충돌 정책을 적용한다.
4) 분기/병합 규칙을 요약 정책과 연결한다.
