...
- 인메모리 체크포인터 구현: `src/secondsession/core/common/checkpointer/inmemory_checkpointer.py`
- 그래프 컴파일 지점: `src/secondsession/core/chat/graphs/chat_graph.py`

---

## 7. 실습 최소 순서(권장)

1) `save`에서 버전 증가 및 `checkpoint_id` 생성 규칙을 구현한다.  
2) `load_latest`로 최신 스냅샷을 가져오는 로직을 완성한다.  
3) `keep_last` 정책으로 오래된 스냅샷을 제거한다.  
4) 멀티 스레드 환경을 고려한 간단한 락 정책을 문서화한다.
